window.addEventListener('load', function(){
  ...
  for(var i=0; i<svgArr.length; i++){
    ...
    // var longestPathLength = getLongestLength(svgGroup);
    // hideSvgFill(svgGroup);
    //
    //hideSvgFill(svgGroup);
    svgGroup.style.strokeDasharray = svgJSON[i].longestLength;
    // svgJSON.push({'element': svgGroup, 'longestLength': Math.floor(longestPathLength), 'dashOffset': Math.floor(longestPathLength)});
    // if(longestPathLength > longestSvgLength){
    //   longestIndex = i;
    //   longestSvgLength = longestPathLength;
    // }
  }
  ...
  // var lightBulb = svgArr[0].children[2].children[0].children[0];
